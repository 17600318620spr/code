...
printf(__func__);
#if 0
fpp=fopen("spr.txt", "ab");
int sprrd=fread(buff, 1, 2048, fp);
int sprwt=fwrite(buff, 1, 2048, fpp);
printf("sprrd=%d sprwt=%d\n", sprrd, sprwt);
fclose(fpp);
#endif
	/* 发文件名 */
...
//fpp=fopen("tzs.txt", "ab");
	...
	while(!feof(fp))
	{
		...
		cnt+=len;
		printf("cnt=%d freadbuf=%s\n", cnt, buf);
 
		printf("Server read %s, len %d\n", filename, len);
//fwrite(buf, 1, len, fpp);
//fwrite(buf, 1, 1024, fpp);
 
		ret = send(sock, buf, len, 0);
		printf("ret=%d sendbuf=%s\n\n", ret, buf);
		if(ret < 0)
		{
